...
export default function Header() {
    ...
    return (
        <header className="fixed top-3 sm:top-4 left-0 right-0 z-50 px-3 sm:px-4 flex justify-center pointer-events-none">
            {/* Main Floating Bar */}
            <nav className="pointer-events-auto w-full max-w-6xl rounded-2xl sm:rounded-full border border-neutral-200/80 dark:border-white/10 bg-white/70 dark:bg-neutral-950/70 backdrop-blur-xl px-4 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between shadow-xl shadow-black/5 dark:shadow-black/40 transition-all duration-300">
                {/* Brand Logo */}
                <Link 
                    href="/" 
                    className="flex items-center gap-2.5 font-bold text-base sm:text-lg hover:opacity-80 transition-opacity"
                    onClick={() => setMobileMenuOpen(false)}
                >
                    <div className="p-1.5 rounded-xl bg-teal-500/10 text-teal-600 dark:text-teal-400 border border-teal-500/20">
                        <Clock className="w-4 h-4 sm:w-5 sm:h-5" />
                    </div>
                    <span className="font-extrabold tracking-tight text-neutral-900 dark:text-white max-sm:hidden">
                        KH DateTime
                    </span>
                    <span className="text-[10px] font-mono font-bold bg-teal-500/10 text-teal-600 dark:text-teal-400 px-2 py-0.5 rounded-full border border-teal-500/20 max-sm:hidden">
                        v{pkg.version}
                    </span>
                </Link>
                ...
                <div className="hidden sm:flex items-center gap-1.5 md:gap-2">
                    <Link
                        ...
                        className={`text-sm font-bold px-3 py-1.5 rounded-full transition-all ${
                            !isCalendar
                                ...
                                : "text-neutral-600 dark:text-neutral-300 hover:text-teal-600 dark:hover:text-teal-400 hover:bg-black/5 dark:hover:bg-white/5"
                        }`}
                    >
                        Home
                    </Link>
                    ...
                    <a
                        href="/#faq"
                        className="text-sm font-bold text-neutral-600 dark:text-neutral-300 hover:text-teal-600 dark:hover:text-teal-400 hover:bg-black/5 dark:hover:bg-white/5 px-3 py-1.5 rounded-full transition-all"
                    >
                        FAQ
                    </a>
                </div>
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <button
                        ...
                        className="sm:hidden p-2 rounded-xl bg-neutral-100 dark:bg-white/5 border border-neutral-200 dark:border-white/10 text-neutral-800 dark:text-neutral-200 hover:bg-neutral-200 dark:hover:bg-white/10 transition-colors"
                    >
            ...
            {mobileMenuOpen && (
                <div className="sm:hidden pointer-events-auto absolute top-16 left-3 right-3 rounded-2xl border border-neutral-200/80 dark:border-white/10 bg-white/95 dark:bg-neutral-950/95 backdrop-blur-2xl p-3 shadow-2xl shadow-black/20 flex flex-col gap-1 animate-in fade-in slide-in-from-top-3 duration-200">
                    {navLinks.map((item) => {
